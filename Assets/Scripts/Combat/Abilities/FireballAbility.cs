...
public class FireballAbility : Ability{
    ...
    public void fireballExplosion(Vector3 spawnPosition){
        ...
        if (Vector3.Distance(spawnPosition, nearestEnemyDistance) < 9f){
            ...
            Player.instance.GetComponent<Sounds3D>().Play("Explosion");
    ...
    public override IEnumerator Ready(){
        ...
        if (Player.instance.mana >= StatDictionary.dict[name][3] + Player.instance.level && Physics.Raycast(ray, out hit, 1000, moveMask)){
    ...
    public override IEnumerator Active(){
        if (AbilityHolder.fireballAnimationReady){
            ...
            Player.instance.PlayerToMouseRotation();
            Player.instance.GetComponent<Sounds3D>().Play("Fireball_cast");
